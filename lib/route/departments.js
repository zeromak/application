...
function promise_to_extract_company_and_department(req) {
  ...
  .then(function(c){
    ...
    return company.getDepartments({
      scope : ['with_simple_users', 'with_boss', 'with_supervisors', 'with_secondary_users'],
      where : {
        id : department_id,
      }
    });
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(){
    ...
    } else if ( req.body.remove_secondary_user_id ) {
      return promise_to_remove_secondary_user({
        user_id    : req.body.remove_secondary_user_id,
        company    : company,
        department : department,
      })
      .then(() => {
        req.session.flash_message('Secondary user was removed from ' + department.name);
        return Promise.resolve(1);
      });
    ...
    } else if ( req.body.do_add_secondary_user ) {
      return promise_to_update_secondary_users({
        req        : req,
        company    : company,
        department : department,
      })
      .then(() => {
        req.session.flash_message('List of secondary users were updated for department ' + department.name);
        return Promise.resolve();
      });
    }
  ...
  .catch(function(error){
    console.error(
      ...
      +' by user '+req.user.id + ' : ' + error + ' at ' + error.stack
    );
...
function promise_to_remove_secondary_user (args) {
  const
    user_id    = args.user_id,
    company    = args.company,
    department = args.department;

  // Make sure that provided user ID belongs to user from current company
  if (company.users.map(u => String(u.id)).indexOf( String(user_id) ) === -1){
    return Promise.resolve(1);
  }

  return department.Model.sequelize.models.UserSecondaryDepartment.destroy({
    where : {
      department_id : department.id,
      user_id       : user_id,
    },
...
function promise_to_update_secondary_users(args) {
  ...
  let secondary_user_ids = req.body.secondary_user_id || [];

  // Take list of all users as a base of intersaction,
  // so we use submitted data only as criteria and do not save it in database
  secondary_user_ids = company.users
    .map(user => user.id)
    .filter(id => secondary_user_ids.indexOf(String(id)) !== -1);

  var link_model = department.Model.sequelize.models.UserSecondaryDepartment;
  ...
  .then(() => link_model.bulkCreate(
    secondary_user_ids.map(id => ({ user_id : id, department_id : department.id  }))
  ));
}
...
router.get('/departments/available-secondary-users/:department_id/', function(req, res){

  let
    department_id = req.param('department_id'),
    department,
    company;

  Promise

  .try(() => promise_to_extract_company_and_department(req))

  .then(result => {
    department = result.department;
    company    = result.company;

    return department.promise_me_with_secondary_users();
  })
  ...
  .then(function(department){
    ...
    department.secondary_users.forEach(user => supervisor_map[user.id] = true);

    res.render('department/available_secondary_users', {
      layout : false,
      users  : _.map(
        _.filter(company.users, user => user.id !== department.bossId),
        user => { user._marked = supervisor_map[user.id]; return user }
      ),
    });
  })
  .catch(error => {
    console.error(
      'An error occurred when trying to get all availabele secondary users for department '+department_id
      +' for user '+req.user.id + ' : ' + error + ' at ' + error.stack
    );

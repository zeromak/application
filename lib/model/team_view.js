...
TeamView.prototype.promise_team_view_details = function(args){
  ...
  var normalise_departments_func = function(my_department, supervised_departments, secondary_departments){

    // Start to build array of resulting departments: based on supervised departments
    let departments = [].concat( supervised_departments );

    if (my_department) {
      // Get all related departments by combining supervised ones with
      // one current user belongs to
      departments.push(my_department);
    }

    // Stick secondary departments into resulting list
    if (Array.isArray(secondary_departments)) {
      departments = departments.concat( secondary_departments );
    }

    // Ensure we do not have duplications
    departments = _.uniq(departments, d => d.id );

    // Copy all available departments for current user into closured variable
    // to pass it into template
    related_departments = _.sortBy(departments, 'name');

    // Find out what particular department is active now
    if (current_department_id) {
      current_department = _.findWhere(departments, { id : Number(current_department_id) });
    }

    return Promise
      .resolve(current_department ? [current_department] : departments);
  }
  ...
  if (user.is_admin() || user.company.share_all_absences) {
    ...
    promise_departments = Promise.join(
      ...
      user.promise_departments_derived_from_secondary_department(),
      normalise_departments_func
    );
  }

  // Calculate users and leaves for every department
  let promise_users_and_leaves = promise_departments
  ...
  return promise_users_and_leaves.then(users_and_leaves => {
    ...
    users_and_leaves = _.uniq( users_and_leaves, it => it.user.id );

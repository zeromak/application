...
TeamView.prototype.promise_team_view_details = function(args){
  ...
  var normalise_departments_func = function(my_department, supervised_departments, secondary_departments){
    ...
    // Stick socendar departments into supervised deparments list
    if (Array.isArray(secondary_departments)) {
      secondary_departments.forEach( dep => supervised_departments.push( dep ) );
    }
    ...
    return Promise
      .resolve(current_department ? [current_department] : supervised_departments)
      .then(departments => {
...
//      user.promise_secondary_departments(),

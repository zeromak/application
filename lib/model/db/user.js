...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    /*
     *  Get list of departments that are derived from secondary
     *  ones.
     *
     *  It get all users that share the same secondary
     *  departments as current user.
     *
     *  Then for that list of users it gets list of their main
     *  departments.
     *
     *  The last list of department is returned.
     *
     * */
    promise_departments_derived_from_secondary_department : function(){
      let self = this;

      return sequelize.models.UserSecondaryDepartment
        .findAll({ where : { user_id : self.id } })
        .then(links => links.map( obj => obj.department_id))
        .then( secondary_department_ids => {
          if ( ! secondary_department_ids ) {
            return Promise.resolve([]);
          }

          // get users who share the same secondary department
          // as current user
          return sequelize.models.UserSecondaryDepartment
            .findAll({ where : {department_id : secondary_department_ids} })
            .then(usds => Promise.resolve(usds.map(usd => usd.user_id)))
        })
        .then(user_ids => sequelize.models.User.findAll({where : { id : user_ids }}))
        // get department IDs
        .then(users => users.map(u => u.DepartmentId))
    ...
    /*
     *  Returns list of departments current user belongs to,
     *  either via "main department" or "secondary department" links
     *
     * */
    promise_departments_I_belong_to : function() {
      const self = this;

      return Promise.join(
        self.getDepartment(),
        self.promise_departments_derived_from_secondary_department(),
        (primary_department, secondary_departments) => sequelize.models.Department
          .scope('with_simple_users')
          .findAll({
            where : { id : [primary_department.id].concat(secondary_departments.map(d => d.id)) }
          })
      );
    },
